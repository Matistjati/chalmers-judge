...
class ScoringWithCodegolf(ScoreboardMaker):
    ...
    def format_tiebreak(self, num_bytes: int) -> str:
        if num_bytes==100000000:
            return "0 bytes"
        return f"{str(num_bytes)} bytes"
        ...
        for sub in submissions:
            ...
            for file, content in sub.submission_files['files'].items():
                ...
                for i in range(len(a)):
                    if a[i]==13 and b[i]==10:
                        num_windows_newline+=1

                num_bytes += len(a)-num_windows_newline
            if score == best_score:
                problem_result.tiebreak = min(problem_result.tiebreak, num_bytes)
            elif score > best_score:
                problem_result.tiebreak = num_bytes
...
class LegacyCodegolf(ScoreboardMaker):
    def __init__(self, contest: Contest, problems: List[ContestProblem], teams: List[Team], **kwargs):
        super(LegacyCodegolf, self).__init__(contest, problems, teams, tiebreak=max, **kwargs)
        self.problem_subtasks = {}
        for problem, scoreboard_problem in zip(self.problems, self.scoreboard_problems):
            subtask_scores = get_subtask_scores(problem.problem.current_version)
            self.problem_subtasks[problem.problem_id] = subtask_scores
...
class ScoringByRuntime(ScoreboardMaker):
    def __init__(self, contest: Contest, problems: List[ContestProblem], teams: List[Team], **kwargs):
        super(ScoringByRuntime, self).__init__(contest, problems, teams, tiebreak=sum, **kwargs)
        self.problem_subtasks = {}
        for problem, scoreboard_problem in zip(self.problems, self.scoreboard_problems):
            subtask_scores = get_subtask_scores(problem.problem.current_version)
            self.problem_subtasks[problem.problem_id] = subtask_scores
    ...
    def format_tiebreak(self, millis: int) -> str:
        if millis==100000000:
            return "0 ms"
        return f"{str(millis)} ms"
